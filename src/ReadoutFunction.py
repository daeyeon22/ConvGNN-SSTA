...
class ReadoutFunction(nn.Module):
    ...
    def r_mpnn(self, h):
        ...
        aux = Variable( torch.Tensor(self.args['out']).type_as(h[0].data).zero_() )
        # modified by dykim 
        ...
        for i in range(h[0].size(0)):
            ...
            # Delete virtual nodes
            nn_res = (torch.unsqueeze(torch.sum(h[0][i,:,:],1),1).expand_as(nn_res)>0).type_as(nn_res)* nn_res
    ...
    def init_mpnn(self, params):
        ...
        # i
        learn_modules.append(NNet(n_in=2*params['in'], n_out=params['target'],\
                dev0 = params['dev0'], dev1 = params['dev1'], dropout = params['dropout'], hlayers = params['layers']))
